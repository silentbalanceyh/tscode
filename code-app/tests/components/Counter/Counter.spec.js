...
describe('(Component) Counter', () => {
  ...
  it('Should events as a <div>.', () => {
    expect(_wrapper.is('div')).to.equal(true)
  })

  it('Should events with an <h2> that includes Sample Counter text.', () => {
    expect(_wrapper.find('h2').text()).to.match(/Counter:/)
  })

  it('Should events props.counter at the end of the sample counter <h2>.', () => {
    expect(_wrapper.find('h2').text()).to.match(/5$/)
    _wrapper.setProps({ counter: 8 })
    expect(_wrapper.find('h2').text()).to.match(/8$/)
  })

  it('Should events exactly two buttons.', () => {
